...
python -m iara_stt_training.evaluate_export  --model $STT_TEST_MODEL --csv "data/smoke_test/ldc93s1.csv" --dump /tmp/result_wav
cat /tmp/result_wav.out

# Evaluate tflite model on opus files
opus_csv="data/smoke_test/ldc93s1_opus.csv"
python -m iara_stt_training.evaluate_export  --model $STT_TEST_MODEL --csv $opus_csv --dump /tmp/result_opus
