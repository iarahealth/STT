               ...
               float default_beta)
{
    ...
    if (!force_bytes_output_mode.value() && !checkpoint_path.has_value()) {
        cerr << "No --checkpoint path specified, not using bytes output mode, can't continue."
             ...
             << "\nStart by creating an alphabet for your models using iarahealth_stt_training.util.check_characters if needed.\n"
             << "\n    python -m iarahealth_stt_training.util.check_characters \\"
             << "\n				--csv-files ... \\"
